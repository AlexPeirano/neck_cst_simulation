...
for chemin in fichiers:
	...
	rm_valeur = extraire_rm(chemin)
	label = f"R_m={rm_valeur}" if rm_valeur else chemin.stem

	rm_float = None
	if rm_valeur is not None:
		try:
			rm_float = float(rm_valeur)
		except ValueError:
			rm_float = None

	if rm_float is not None and abs(rm_float - 5.0) < 1e-6:
		ax.plot(
			freq_ghz,
			s11_db,
			linewidth=3.0,
