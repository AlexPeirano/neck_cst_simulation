...
for chemin in fichiers:
	...
	label = f"y_offset={y_offset_valeur}" if y_offset_valeur else chemin.stem

	y_offset_float = None
	if y_offset_valeur is not None:
		try:
			y_offset_float = float(y_offset_valeur)
		except ValueError:
			y_offset_float = None

	if y_offset_float is not None and abs(y_offset_float - (-3.0)) < 1e-6:
		ax.plot(
			freq_ghz,
			s11_db,
			linewidth=3.0,
			color="#d62728",
			label=f"{label} (y_offset=-3)",
			zorder=5,
		)
	else:
