...
# List and sort the simulation files
...
def plot_slice(XG, ZG, slice_interp, title, ylabel, xlabel, cbar_label, clim, cmap, output_name, save_png=True):
    ...
    # heatmap using imshow for better visual smoothness
    norm = Normalize(*clim)
    ...
    for th, name in zip(thick, names):
        z_end = current_z + th    # calculates where the current layer ends 
        mid_z = (current_z + z_end) / 2  # calculates the middle of the current layer
        is_csf = (name == 'CSF')
        ...
        if is_csf:
            ax.axhline(current_z, color='red', linewidth=1.5)  # to draw the first line of the CSF 
...
for fname in files:
    ...
    # Use 'cubic' interpolation if possible for maximum smoothness
    try:
        interp_lin = RegularGridInterpolator((Z_phys, X), slice_lin, method='cubic', bounds_error=False, fill_value=0)
    except ValueError:
    ...
    try:
        interp_db = RegularGridInterpolator((Z_phys, X), slice_db, method='cubic', bounds_error=False, fill_value=-120)
    except ValueError:
