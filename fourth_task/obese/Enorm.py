import os
import re
import glob
import numpy as np
import h5py
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
from matplotlib import colors as mpl_colors
from matplotlib.colors import Normalize
from scipy.interpolate import RegularGridInterpolator
from PIL import Image
import io

# Declaration of the model's geommetry
# Obese: tfat=7.7, tmuscle=34.5
thick = [1.6, 4.0, 1.4, 7.7, 34.5, 13.85, 2.2, 10.0, 10.0, 2.2, 13.85, 34.5, 7.7, 1.4, 4.0, 1.6]
names = ['Substrate', 'Coupling Medium', 'Skin', 'Fat', 'Muscle', 'Bone',
         'CSF', 'Spinal Cord', 'Spinal Cord', 'CSF', 'Bone', 'Muscle', 'Fat', 'Skin', 'Coupling Medium', 'Substrate']

# List and sort simulation files
script_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(
    glob.glob(os.path.join(script_dir, 'e-field*.h5')),
    key=lambda x: float(re.search(r'\d+\.?\d*', os.path.basename(x)).group())
)
if not files:
    raise FileNotFoundError(f'No e-field*.h5 file found in: {script_dir}')

from parula_cmap import get_parula_cmap
parula_cmap = get_parula_cmap()

def plot_slice(XG, ZG, slice_interp, title, ylabel, xlabel, cbar_label, clim, cmap, output_name, save_png=True):
    fig, ax = plt.subplots(figsize=(11, 8), facecolor='white')
    fig.subplots_adjust(left=0.08, right=0.72, bottom=0.10, top=0.92)

    norm = Normalize(*clim)
    im = ax.imshow(slice_interp, 
                   extent=[XG.min(), XG.max(), ZG.min(), ZG.max()],
                   origin='lower', aspect='auto', 
                   cmap=cmap, norm=norm, 
                   interpolation='bicubic')

  
    total_thick = sum(thick)
    current_z = -total_thick / 2
    
    # Draw the very first interface
    ax.axhline(current_z, color='white', linewidth=0.8)

    for th, name in zip(thick, names):
        z_end = current_z + th
        mid_z = (current_z + z_end) / 2
        is_csf = (name == 'CSF')
        color = 'red' if is_csf else 'white'
        lw = 1.5 if is_csf else 0.8

        ax.axhline(z_end, color=color, linewidth=lw)
        if is_csf:
            ax.axhline(current_z, color='red', linewidth=1.5)

        ax.text(29.5, mid_z, name, fontsize=9, color='red' if is_csf else 'black',
                va='center', fontweight='bold', clip_on=False)
        current_z = z_end

    ax.set_xlim(-28, 28)
    ax.set_ylim(-total_thick / 2 - 5, total_thick / 2 + 5)
    ax.set_title(title, fontsize=12, fontweight='bold')
    ax.set_ylabel(ylabel, fontsize=10)
    ax.set_xlabel(xlabel, fontsize=10)

    cbar_ax = fig.add_axes([0.88, 0.10, 0.02, 0.82])
    cb = fig.colorbar(im, cax=cbar_ax)
    cb.set_label(cbar_label, fontweight='bold', fontsize=9)

    if save_png:
        results_dir = os.path.join(script_dir, "results")
        os.makedirs(results_dir, exist_ok=True)
        fig.savefig(os.path.join(results_dir, output_name), dpi=150)
    
    
    buf = io.BytesIO()
    fig.savefig(buf, format='png', dpi=100)
    buf.seek(0)
    img = Image.open(buf).copy()
    plt.close(fig)
    return img

# Find the maximum value in Y-plane across all frequencies
E_max_slice = 0.0
for fname in files:
    with h5py.File(fname, 'r') as f:
        Y = f['/Mesh line y'][:]
        iy = np.argmin(np.abs(Y))
        E = f['/E-Field']
        Ex = E['x']['re'][:, iy, :] + 1j * E['x']['im'][:, iy, :]
        Ey = E['y']['re'][:, iy, :] + 1j * E['y']['im'][:, iy, :]
        Ez = E['z']['re'][:, iy, :] + 1j * E['z']['im'][:, iy, :]
        mag_slice = np.sqrt(np.abs(Ex)**2 + np.abs(Ey)**2 + np.abs(Ez)**2)
        local_max = np.nanmax(mag_slice)
        if local_max > E_max_slice:
            E_max_slice = local_max
print(f'Slice maximum: {E_max_slice:.3e} V/m')

frames_lin = []
frames_db = []

for fname in files:
    print(f'Processing: {os.path.basename(fname)}')
    freq_str = re.search(r'\d+\.?\d*', os.path.basename(fname)).group()
    
    with h5py.File(fname, 'r') as f:
        X = f['/Mesh line x'][:]
        Y = f['/Mesh line y'][:]
        Z = f['/Mesh line z'][:]
        E_raw = f['/E-Field']
        iy = np.argmin(np.abs(Y))
        Ex = E_raw['x']['re'][:, iy, :] + 1j * E_raw['x']['im'][:, iy, :]
        Ey = E_raw['y']['re'][:, iy, :] + 1j * E_raw['y']['im'][:, iy, :]
        Ez = E_raw['z']['re'][:, iy, :] + 1j * E_raw['z']['im'][:, iy, :]
        slice_lin = np.sqrt(np.abs(Ex)**2 + np.abs(Ey)**2 + np.abs(Ez)**2)

    # Depth calibration: center the tissue block on 0.
    total_thick = sum(thick)
    z_center_offset = total_thick / 2 - thick[0]
    Z_phys = Z - Z[24] - z_center_offset

    x_grid = np.linspace(-28, 28, 800)
    z_grid = np.linspace(-total_thick / 2 - 5, total_thick / 2 + 5, 1000)
    ZG, XG = np.meshgrid(z_grid, x_grid, indexing='ij')
    interp_pts = np.stack([ZG, XG], axis=-1)


    # Normalized Linear
    slice_norm_lin = slice_lin / E_max_slice
    try:
        interp_lin = RegularGridInterpolator((Z_phys, X), slice_norm_lin, method='cubic', bounds_error=False, fill_value=0)
    except ValueError:
        interp_lin = RegularGridInterpolator((Z_phys, X), slice_norm_lin, method='linear', bounds_error=False, fill_value=0)
    slice_interp_lin = interp_lin(interp_pts)
    
    img_lin = plot_slice(XG, ZG, slice_interp_lin, 
                        f'Normalized E-Field (Linear) at {freq_str} GHz',
                        'Depth (mm)', 'Lateral Position (mm)', '|E| / |E|_max',
                        (0, 1), parula_cmap, 
                        f'efield_norm_lin_{freq_str}GHz.png', save_png=True)
    frames_lin.append(img_lin)

    # Normalized dB
    eps = 1e-12
    slice_norm_db = 20.0 * np.log10(np.maximum(slice_norm_lin, eps))
    try:
        interp_db = RegularGridInterpolator((Z_phys, X), slice_norm_db, method='cubic', bounds_error=False, fill_value=-120)
    except ValueError:
        interp_db = RegularGridInterpolator((Z_phys, X), slice_norm_db, method='linear', bounds_error=False, fill_value=-120)
    slice_interp_db = interp_db(interp_pts)
    
    img_db = plot_slice(XG, ZG, slice_interp_db, 
                       f'Normalized E-Field (dB) at {freq_str} GHz',
                       'Depth (mm)', 'Lateral Position (mm)', '20*log10(|E|/|E|_max)',
                       (-50, 0), parula_cmap, 
                       f'efield_norm_db_{freq_str}GHz.png', save_png=True)
    frames_db.append(img_db)

# Export GIFs to 'gifs' folder
gif_dir = os.path.join(script_dir, "gifs")
os.makedirs(gif_dir, exist_ok=True)
if frames_lin:
    frames_lin[0].save(os.path.join(gif_dir, 'efield_norm_linear.gif'), save_all=True, append_images=frames_lin[1:], loop=0, duration=1000)
if frames_db:
    frames_db[0].save(os.path.join(gif_dir, 'efield_norm_dB.gif'), save_all=True, append_images=frames_db[1:], loop=0, duration=1000)

print("Normalized processing complete.")
